...
const Login = () => {
  ...
  const handleEdit = (jokeId, jokeText) => {
    setEditingJokeId(jokeId);
    setEditedJokeText(jokeText);
  };

  const handleUpdateJoke = async (jokeId, description) => {
    try {
      const response = await updateJoke(token, jokeId, description);
      console.log(response); // Optionally handle the response
      // After updating the joke, you might want to refresh the list
      handleList();
      // Clear editing state
      ...
      {token ? (
        <>
          ...
          {loading ? (
            ...
            <ul className="p-0">
              {adminList.map((item) => (
                <div className="card m-3 bg-color-red" key={item._id}>
                    ...
                    <div className="col-auto">
                      {editingJokeId === item._id ? (
                        <button
                          ...
                          onClick={() => handleUpdateJoke(item._id, editedJokeText)}
                        >
                          Update
                        </button>
                      ) : (
